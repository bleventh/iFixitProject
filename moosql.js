  ...
  MooSQL = new Class({
    ...
    findAll: function(tables, callback) {
      ...
      $splat(tables).each(function(item, i) {
        if (i === 0) {
          return query += "SELECT *, '" + item + "' AS type FROM '" + item + "' ";
        } else {
          return query += "union SELECT *, '" + item + "' AS type FROM '" + item + "' ";
        }
      });
    ...
    save: function(table, set, Where, callback) {
      return this.find(table, '*', Where, (function(tr, result) {
        if (result.rows.length > 0) {
          return this.update(table, set, Where, callback);
        } else {
          return this.insert(table, $extend(set, Where), callback);
        }
      }).bindWithEvent(this));
    },
